        ...
        <div class="info-zone">
          ...
          <div class="meta-dashboard">
            <span class="badge rating"
              >⭐ {{ store.selectedMovie?.vote_average.toFixed(1) }} / 10</span
            >
            <span class="badge runtime"
              >⏱️ {{ store.selectedMovie?.runtime }}분</span
            >
            <span class="badge release"
              >📅 {{ store.selectedMovie?.release_date }} 개봉</span
            >
          </div>
...
const generateAIReview = async () => {
  ...
  try {
    const promptMessage = `너는 영화 평론 유튜버야. 아래 영화 데이터를 기반으로 블로그 글처럼 2~3문단 분량의 상세하고 흡입력 있는 추천평을 작성해줘(이모지 필수).
...
.ai-btn:hover {
  background: linear-gradient(45deg, #9b4de0, #5c009e);
}
